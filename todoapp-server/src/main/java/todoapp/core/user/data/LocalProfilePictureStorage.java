...
class LocalProfilePictureStorage implements ProfilePictureStorage, ResourceLoaderAware, InitializingBean {
    ...
    public URI save(Resource resource) {
        try {
            ...
            throw new FailedSavingProfilePictureException("failed to save profile picture to local disk", error);
        }
    }
    ...
    public Resource load(URI uri) {
        try {
            ...
            throw new FailedLoadingProfilePictureException("failed to load profile picture to local disk", error);
        }
    }

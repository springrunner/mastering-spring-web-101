...
public class LocalProfilePictureStorage implements ProfilePictureStorage, ResourceLoaderAware, InitializingBean {
  ...
  public URI save(Resource resource) {
    try {
      ...
      throw new FailedSavingProfilePictureException("로컬 디스크에 프로필 이미지 저장 중 오류가 발생했습니다.", error);
    }
  }
  ...
  public Resource load(URI uri) {
    try {
      ...
      throw new FailedLoadingProfilePictureException("로컬 디스크에 프로필 이미지 불러오기 중 오류가 발생했습니다.", error);
    }
  }

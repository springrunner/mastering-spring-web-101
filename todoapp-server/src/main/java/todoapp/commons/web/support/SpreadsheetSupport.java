...
public interface SpreadsheetSupport {
  ...
  default Spreadsheet obtainSpreadsheet(Map<String, Object> model) {
    ...
    return (Spreadsheet) spreadsheets.get(0);
  }
  ...
  default String joining(Iterable<Spreadsheet.Cell<?>> cells, CharSequence delimiter) {
    return StreamUtils.createStreamFromIterator(cells.iterator()).map(Spreadsheet.Cell::getValue).map(String::valueOf).collect(Collectors.joining(delimiter));
  }

...
public class TodoToSpreadsheetConverter {
  ...
  public Spreadsheet convert(Iterable<Todo> todos) {
    Spreadsheet.Row header = new Spreadsheet.Row().addCell("id").addCell("title").addCell("completed");
  ...
  private Spreadsheet.Row mapRow(Todo todo) {
    return new Spreadsheet.Row().addCell(todo.getId()).addCell(todo.getTitle()).addCell(todo.isCompleted() ? "완료" : "미완료");
  }

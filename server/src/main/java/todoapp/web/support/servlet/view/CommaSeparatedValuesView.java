...
import todoapp.core.foundation.domain.Spreadsheet;
import todoapp.web.support.SpreadsheetSupport;
...
public class CommaSeparatedValuesView extends AbstractView implements SpreadsheetSupport {
    ...
    protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        var spreadsheet = obtainSpreadsheet(model);
        ...
        if (spreadsheet.hasHeader()) {
            var header = spreadsheet.getHeader().map(row -> joining(row.getCells(), ",")).orElse("");
            response.getWriter().println(header);
        }

        if (spreadsheet.hasRows()) {
            for (var row : spreadsheet.getRows()) {
                response.getWriter().println(joining(row.getCells(), ","));
            }
        }
